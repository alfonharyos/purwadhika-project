-- back compat for old kwarg name
    ...
    merge into `purwadika`.`jcdeol004_alfon_movie_streaming_mart`.`mart_subscription_revenue_monthly` as DBT_INTERNAL_DEST
        using (
...
left join user_activity ua
    on r.month_key = ua.month_key
        ...
        on (
                DBT_INTERNAL_SOURCE.subscription_month_key = DBT_INTERNAL_DEST.subscription_month_key
            )

    
    when matched then update set
        `subscription_id` = DBT_INTERNAL_SOURCE.`subscription_id`,`month_key` = DBT_INTERNAL_SOURCE.`month_key`,`total_amount` = DBT_INTERNAL_SOURCE.`total_amount`,`total_payments` = DBT_INTERNAL_SOURCE.`total_payments`,`status_completed` = DBT_INTERNAL_SOURCE.`status_completed`,`status_pending` = DBT_INTERNAL_SOURCE.`status_pending`,`status_failed` = DBT_INTERNAL_SOURCE.`status_failed`,`subscription_month_key` = DBT_INTERNAL_SOURCE.`subscription_month_key`,`run_date_bq` = DBT_INTERNAL_SOURCE.`run_date_bq`,`total_active_users` = DBT_INTERNAL_SOURCE.`total_active_users`,`new_users` = DBT_INTERNAL_SOURCE.`new_users`,`churned_users` = DBT_INTERNAL_SOURCE.`churned_users`
    

    when not matched then insert
        (`subscription_id`, `month_key`, `total_amount`, `total_payments`, `status_completed`, `status_pending`, `status_failed`, `subscription_month_key`, `run_date_bq`, `total_active_users`, `new_users`, `churned_users`)
    values
        (`subscription_id`, `month_key`, `total_amount`, `total_payments`, `status_completed`, `status_pending`, `status_failed`, `subscription_month_key`, `run_date_bq`, `total_active_users`, `new_users`, `churned_users`)

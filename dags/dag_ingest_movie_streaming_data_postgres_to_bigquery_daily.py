...
def ingestion_pg_to_bq(pg_table_name: str, stg_table_name: str, partition_field: str, pg_conn: str = "movie_db", **kwargs):
    ...
    day_minus_1  = execution_date - timedelta(days=1)
    df = extract_postgres_to_df(
        query = f"""
            SELECT * FROM {pg_table_name}
            WHERE DATE(created_at) = DATE('{day_minus_1}')
        """,
        conn_id=pg_conn,
        logger=logger
    )
...
# ---------- DAG ----------
with DAG(
    dag_id="dag_ingest_movie_streaming_pg_to_bq_daily",
    description='Ingestion Movie Streaming daily data to BigQuery (stg -> raw)',
    tags=['daily', 'ingestion', 'movie_streaming', 'bigquery'],
    on_failure_callback=dag_failure_alert,
    on_success_callback=dag_success_alert,
    start_date=pendulum.datetime(2025, 9, 20, tz=local_tz),
